...
(function() {
    ...
    window.initPhotoColorsPanel = function(obj) {
        ...
        filters.forEach(function(f) {
            if (f.type === 'Brightness') brightness = (f.brightness + 1) * 50;
            else if (f.type === 'Contrast') contrast = (f.contrast + 1) * 50;
            else if (f.type === 'Saturation') saturation = (f.saturation + 1) * 100;
            else if (f.type === 'HueRotation') hue = f.rotation || 0;
            else if (f.type === 'Grayscale') grayscale = true;
            else if (f.type === 'Sepia') sepia = (f.amount || 0) * 100;
        });
    ...
    function applyPhotoColorFilters() {
        ...
        obj.filters = obj.filters.filter(function(f) {
            return ['Brightness','Contrast','Saturation','HueRotation','Grayscale','Sepia'].indexOf(f.type) === -1;
        });
