...
@section('title', 'Template Licenses')
@section('page-title', 'Template Licenses')
...
<div class="my-4">
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
            <h2 class="mb-1" style="font-weight: 700; font-size: 1.5rem; color: #1e293b;">
                <i class="fas fa-certificate me-2 text-primary"></i>Template Licenses
            </h2>
            <p class="text-muted mb-0">Manage template license types (Standard, Extended, Commercial, etc.)</p>
        </div>
    ...
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-body">
            <form method="GET" action="{{ route('admin.licenses') }}" class="row g-3">
                <div class="col-md-6">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td style="padding: 1rem;">
                                <div style="font-size: 0.9rem; font-weight: 500; color: #1e293b;">{{ $license->name }}</div>
                            </td>
                            <td style="padding: 1rem;">
                                <code style="background: #f1f5f9; padding: 0.25rem 0.5rem; border-radius: 4px; font-size: 0.8rem;">{{ $license->slug }}</code>
                            </td>
                            <td style="padding: 1rem; font-size: 0.85rem; color: #64748b;">
                                {{ Str::limit($license->description, 50) ?: '—' }}
                            </td>
                            <td style="padding: 1rem; font-size: 0.85rem; color: #64748b;">{{ $license->sort_order }}</td>
                            <td style="padding: 1rem;">
                            ...
                            <td style="padding: 1rem; font-size: 0.85rem; color: #94a3b8;">{{ $license->created_at->format('M d, Y') }}</td>
                            <td style="padding: 1rem; text-align: center;">
                                <div class="btn-group" role="group">
                                    <a href="{{ route('admin.licenses.edit', $license->id) }}" class="btn btn-sm btn-outline-primary" title="Edit">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <form action="{{ route('admin.licenses.delete', $license->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this license?');">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @empty
                        <tr>
                            <td colspan="7" class="text-center py-4 text-muted">No licenses found. <a href="{{ route('admin.licenses.create') }}">Create one</a></td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
